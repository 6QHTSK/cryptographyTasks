...
void MadeMap(char str[8], char *rainbowStart) {
    node *CurrentNode = &Root;
    for (int i = 0; i < 8; i++) {
        int pos = transBase36(str[i]);
        if (CurrentNode->next[pos] == NULL) {
            node *NewNode = malloc(sizeof(node));
            memset(NewNode, 0, sizeof(node));
            CurrentNode->next[pos] = NewNode;
        }
        CurrentNode = CurrentNode->next[pos];
    }
    CurrentNode->rainbowStart = rainbowStart;
}

char *FindMap(char str[8]) {
    node *CurrentNode = &Root;
    for (int i = 0; i < 8; i++) {
        int pos = transBase36(str[i]);
        if (CurrentNode->next[pos] == NULL) {
    ...
    return CurrentNode->rainbowStart;
}
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        char rainbowEnd[8];
        char *rainbowStart = malloc(sizeof(char) * 8);
        scanf("%s%s", rainbowStart, rainbowEnd);
        MadeMap(rainbowEnd, rainbowStart);
    }
    unsigned int sha1[5], tsha1[5];
    scanf("%8x%8x%8x%8x%8x", &sha1[0], &sha1[1], &sha1[2], &sha1[3], &sha1[4]);
    char t[8], *start = NULL;
    for (int i = 0; i < 100; i++) {
        memcpy(tsha1, sha1, sizeof(unsigned int) * 5);
        for (int j = 0; j < 10000; j++) {
            R(tsha1, t, (i + j) % 100 + 1);
            start = FindMap(t);
            if (start != NULL) {
                char t2[9];
                unsigned int t2sha1[5];
                strcpy(t2, start);
                for (int k = 0; k < 10000; k++) {
                    UnitSHA1(t2, 8, t2sha1);
                    if(sha1[0] == t2sha1[0] && sha1[1] == t2sha1[1] && sha1[2] == t2sha1[2] && sha1[3] == t2sha1[3]&& sha1[4] == t2sha1[4]){
                        printf("%s", t2);
                        return 0;
                    }
                    R(t2sha1, t2, k % 100 + 1);
